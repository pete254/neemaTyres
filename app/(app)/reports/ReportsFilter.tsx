...
export default function ReportsFilter({ fromStr, toStr, today }: { fromStr: string; toStr: string; today: string }) {
  const router = useRouter();
  const [from, setFrom] = useState(fromStr);
  const [to, setTo] = useState(toStr);

  const apply = (f: string, t: string) => router.push(`/reports?from=${f}&to=${t}`);

  const inputCls = "bg-[#1C1C1C] border border-[#2A2A2A] rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-[#EAB308]";
  const navBtn = "px-2 py-2 text-zinc-400 hover:text-white bg-[#1C1C1C] border border-[#2A2A2A] rounded text-sm disabled:opacity-30";

  return (
    <form onSubmit={(e) => { e.preventDefault(); apply(from, to); }} className="mb-6 flex gap-3 items-end flex-wrap">
      <div>
      ...
      <div>
        ...
        <div className="flex items-center gap-1">
          ...
          <button type="button" onClick={() => setTo(shiftDate(to, 1, today))}
            disabled={to >= today} className={navBtn}>›</button>
        </div>
      </div>
      <button type="submit"
