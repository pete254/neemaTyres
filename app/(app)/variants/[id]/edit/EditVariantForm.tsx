...
export default function EditVariantForm({ action, deleteAction, deleteError, brands, variant }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Delete form — must be OUTSIDE the save form above ── */}
      <div className="border border-red-900 bg-red-950/20 rounded-lg p-4 mt-2">
        ...
        {deleteError ? (
          <p className="text-red-300 text-sm bg-red-900/30 border border-red-800 rounded px-3 py-2 mb-3">
            Cannot delete — this tyre has sales, purchases, returns, or opening stock records linked to it.
          </p>
        ) : (
          <p className="text-red-300/60 text-xs mb-3">
            Permanently removes this tyre from inventory. Only possible if it has no sales, purchases, or stock history.
          </p>
        )}
